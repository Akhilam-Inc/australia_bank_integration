				...
				function () {
					frm.call("validate_skript_account_mapping").then((r) => {
						if (r.message) {
							...
							let account_list = r.message
								.map(
									(acc) =>
										`• ${acc.display_name} (${
											acc.masked_number || acc.account_id
										})`
								)
								.join("<br>");
							...
							frappe.msgprint({
								...
								indicator: "green",
								message:
									"✅ All Skript accounts are mapped to ERPNext Bank Accounts!",
							});
						}
					});
				},
