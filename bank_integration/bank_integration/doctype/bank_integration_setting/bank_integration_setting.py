...
class BankIntegrationSetting(Document):
	...
	if TYPE_CHECKING:
		...
		from bank_integration.bank_integration.doctype.airwallex_client.airwallex_client import (
			AirwallexClient,
		)
		from bank_integration.bank_integration.doctype.skript_account.skript_account import SkriptAccount
		from bank_integration.bank_integration.doctype.transaction_type_filter.transaction_type_filter import (
		...
		skript_sync_status: DF.Literal[
			"Not Started", "In Progress", "Completed", "Completed with Errors", "Failed"
		]
	...
	def _to_iso8601(self, dt):
		...
		try:
			...
			frappe.log_error(f"Error converting datetime to ISO8601: {e}", "ISO8601 Conversion Error")
			return None
	...
	def _credentials_changed(self):
		...
		for client_id, client in current_clients.items():
			...
			if old_client:
				...
				if client.get_password("airwallex_api_key") != old_client.get_password("airwallex_api_key"):
					return True
	...
	def get_airwallex_clients(self):
		...
		return [
			client for client in self.airwallex_clients if client.airwallex_client_id and client.bank_account
		]
	...
	def test_authentication_silent(self):
		...
		for client in self.airwallex_clients:
			try:
				...
				frappe.log_error(
					f"Authentication failed for client {client.airwallex_client_id}: {e}",
	...
	def test_authentication(self):
		...
		if success_count == total_clients:
			...
			frappe.msgprint(error_message, indicator="orange", realtime=True, alert=False)
			return False
	...
	def stop_transaction_sync(self):
		...
		try:
			...
			frappe.throw(f"Failed to stop sync job: {e}")
	...
	def update_sync_progress(self, processed, total, status="In Progress"):
		...
		frappe.publish_realtime(
			...
			{"processed": processed, "total": total, "progress": progress, "status": status},
	...
	def fetch_and_create_skript_accounts(self):
		...
		try:
			...
			message_parts.append(
				"<br><br>📝 The page will reload. Please map Skript Accounts to ERPNext Bank Accounts in the table."
			)
			...
			frappe.throw(f"Failed to fetch accounts: {e}")
	...
	def start_skript_transaction_sync(self):
		...
		if unmapped:
			...
			account_list = "<br>".join(
				[f"• {acc['display_name']} ({acc['masked_number'] or acc['account_id']})" for acc in unmapped]
			)
	...
	def _skript_credentials_changed(self):
		...
		if self.get_password("skript_client_id") != old_doc.get_password("skript_client_id"):
			return True

		if self.get_password("skript_client_secret") != old_doc.get_password("skript_client_secret"):
			return True
	...
	def test_skript_authentication_silent(self):
		...
		try:
			...
			frappe.log_error(f"Skript authentication test failed: {e}", "Skript Auth Test")
			return False
